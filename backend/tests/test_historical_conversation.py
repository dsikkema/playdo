...
def test_historical_conversation_new_conversation_flow():
    ...
    def get_next_assistant_resp_side_effect(messages, user_input):
        ...
        user_message = PlaydoMessage(
            role="user",
            content=[{"type": "text", "text": user_input}]
        )
        assistant_message = PlaydoMessage(
            role="assistant",
            content=[{"type": "text", "text": f"Response to: {user_input}"}]
        )
    ...
    historical_conversation = HistoricalConversation(
        ...
        response_getter=mock_response_getter
    )
...
def test_historical_conversation_load_existing_conversation():
    ...
    existing_messages = [
        ...
        PlaydoMessage(
            role="user",
            content=[{"type": "text", "text": "Previous message 2"}]
        ),
        PlaydoMessage(
            role="assistant",
            content=[{"type": "text", "text": "Previous response 2"}]
        )
    ]
